...
int main()
{
    ...
    // Create socket
    if ((EW_client_socket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) == INVALID_SOCKET) 
    {
    ...
    if (inet_pton(AF_INET, "127.0.0.1", &server_address.sin_addr) <= 0) 
    {
        ...
        closesocket(EW_client_socket);
        WSACleanup();
        return -1;
    }

    // Connect to the server
    if (connect(EW_client_socket, (struct sockaddr*)&server_address, sizeof(server_address)) == SOCKET_ERROR) 
    {
    ...
    std::cout << "EW Players connected to server\n";
    ...
    strToCards(cards, E_Player);
    send(EW_client_socket, "first package recived", strlen("first package recived"), 0);
    ...
    strToCards(cards, W_Player);
    send(EW_client_socket, "second package recived", strlen("second package recived"), 0);
    ...
    {
        clearBuffer(buffer, 1024);
        recv(EW_client_socket, buffer, sizeof(buffer), 0); // buffer[0] nazwa gracza buffer[1] wartosc [2] kolor [3] bidujacy aktualnie gracz
        ...
        if (buffer[0] == 'E') // biduje gracz N
        {
            if (Bid(contract, E_Player))
            {
                buffer[0] = 'E';
                buffer[1] = contract[0];
                buffer[2] = contract[1];
                buffer[3] = contract[2];
                send(EW_client_socket, buffer, sizeof(buffer), 0);
            }
            else
            {
                send(EW_client_socket, "pass", strlen("pass"), 0);
            }
        }
        else if (buffer[0] == 'W') // biduje gracz S
        {
            if (Bid(contract, W_Player))
            {
                buffer[0] = 'W';
                buffer[1] = contract[0];
                buffer[2] = contract[1];
            ...
            {
                send(EW_client_socket, "pass", sizeof("pass"), 0);
            }
        }
    }
	...
	while (1)
	{
		char PlayerName;
		clearBuffer(buffer, 1024);
		int val = recv(EW_client_socket, buffer, sizeof(buffer), 0);
		...
		if (PlayerName == 'E')
		{
			playCard(E_Player, contract, PlayedCard);
			informationToStr(PlayedCard, buffer);
			send(EW_client_socket, buffer, strlen(buffer), 0);
		}
		else if (PlayerName == 'W')
		{
			playCard(W_Player, contract, PlayedCard);
			informationToStr(PlayedCard, buffer);
			send(EW_client_socket, buffer, strlen(buffer), 0);
		}
	}
